...
def preprocess_data(df):
    # Map packaging categories to numerical values
    packaging_mapping = {'Cartone S': 0, 'Plastica M': 1, 'Cartone L': 2, 'Plastica S': 3}
    df['packaging'] = df['packaging'].map(packaging_mapping)
    
    # Filter out invalid labels (only keep 0, 1, 2)
    valid_labels = [0, 1, 2]
    df = df[df['packaging'].isin(valid_labels)]
...
packaging_mapping = {0: 'Cartone S', 1: 'Plastica M', 2: 'Cartone L'}
