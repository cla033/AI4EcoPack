...
def auto_train_model(df):
    ...
    modelli = {
        "RandomForest": RandomForestClassifier(n_estimators=300, max_depth=20, min_samples_split=2, random_state=42),
        "GradientBoosting": GradientBoostingClassifier(n_estimators=200, learning_rate=0.05, max_depth=6, random_state=42),
        "ExtraTrees": ExtraTreesClassifier(n_estimators=300, max_depth=None, random_state=42)
    }
    ...
    for nome, modello in modelli.items():
        ...
        score = accuracy_score(y_test, modello.predict(X_test))
        if score > best_score:
            best_score = score
            best_model = modello
...
if st.session_state.get("prediction_done", False):
    ...
    pdf.cell(200, 10, txt="📦 Report AI4EcoPack", ln=True, align="C")
    pdf.set_font("Arial", size=12)
    pdf.ln(5)
    for col in nuovo.columns:
        pdf.cell(200, 10, txt=f"{col}: {nuovo[col].values[0]}", ln=True)
    pdf.ln(5)
    pdf.cell(200, 10, txt=f"📦 Packaging consigliato: {prediction[0]}", ln=True)
    pdf.cell(200, 10, txt=f"🌿 CO2 stimata: {co2_stimato:.2f} grammi", ln=True)

    # Aggiunta grafico
    emissioni_totali = {
        "Cartone riciclato": 1200,
        "Plastica riciclata": 2500,
        "Bioplastica": 900,
        "Polistirolo": 4000,
        "Legno": 1800
    }
    fig, ax = plt.subplots()
    ax.bar(emissioni_totali.keys(), emissioni_totali.values())
    ax.set_ylabel("g CO₂")
    ax.set_xlabel("Tipo di packaging")
    ax.set_title("Inquinamento totale stimato")
    ...
    # Salvataggio finale
    pdf_path = "/mnt/data/ai4ecopack_output.pdf"
    pdf.output(pdf_path)
    with open(pdf_path, "rb") as f:
        st.download_button("📄 Scarica il PDF con i risultati", f, file_name="ai4ecopack_output.pdf")
